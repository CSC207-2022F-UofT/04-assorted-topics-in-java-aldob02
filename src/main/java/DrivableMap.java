...
class DrivableMap {
    ...
    public boolean addDrivable(String id, Drivable item){
        for (String idKey : drivable_map.keySet()) if (id.equals(idKey)) return false;
    ...
    public boolean hasFasterThan (int speed){
        for (Drivable itemValue : drivable_map.values()) if (itemValue.getMaxSpeed() >= speed) return true;
        return false;
    }
    ...
    public List<Tradable> getTradable(){
        ...
        for (Drivable item : drivable_map.values()) if (item instanceof Tradable) tradableItems.add((Tradable) item);
